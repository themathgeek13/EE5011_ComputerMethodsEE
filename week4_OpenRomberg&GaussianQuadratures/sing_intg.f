...
* open form of trapzd.
* n=1 calls func once
* n=2 calls func 2 times (total 3)
* n=3 calls func 6 times (total 9)
* n=4 calls func 18 times (total 27) etc
      SUBROUTINE midpnt(func,a,b,sin,sout,n)
c
...
c
      ...
      ELSE
         ...
         DO j=1,it
            ...
            x=x+del
         END DO
         sout=(sin+(b-a)*sum/tnm)/3.0d0
      ENDIF
      RETURN
      END


      SUBROUTINE qromo(func,a,b,ss,dss,numcalls,EPS,K,c,d)
c
cf2py intent(out) :: ss
...
c     PARAMETER (EPS=1.e-6, JMAX=20, JMAXP=JMAX+1, K=5, KM=K-1)
      ...
      INTEGER KM
      INTEGER j
      REAL*8 dss,h(JMAXP),s(JMAXP),sold,scl
      scl=1.0d0/9.0d0
      h(1)=1.d0
      KM=K-1
      sold=0.0d0
      DO j=1,JMAX
        CALL midpnt(func,a,b,sold,s(j),j)
        IF (j.GE.K) THEN
